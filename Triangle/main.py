...
class Test(MDApp):
    ...
    def doThis(self, *args):
        try:
            side_a = float(self.root.ids.sa.text)
            side_b = float(self.root.ids.sb.text)
            side_c = float(self.root.ids.sc.text)
            ...
            result_label = self.root.ids.res
            result_label.text = f"Perimeter: {perimeter:.2f} m\nArea: {area:.2f} sq.m"
        except ValueError:
